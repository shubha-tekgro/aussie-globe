    ...
    <>
      ...
      <div className='flex justify-center items-center w-full p-2 lg:hidden '>
        <div className='relative flex items-center w-full justify-center md:ml-80 -mt-2'>
          ...
          <button onClick={() => setShowMobileSearch(true)} className='-mt-10 ml-36 z-50 md:right-44'>
            <Image src={SearchIcon} alt='Search' className='h-5 w-20' />
          </button>

          <div className={`relative mt-2 w-full  transition-all duration-500 ease-in-out transform ${showMobileSearch ?
            'translate-x-0 opacity-200' : 'translate-x-20 opacity-0 pointer-events-none md:ml-96 md:mt-2'}`}>

            <input type='text' placeholder='Search' autoFocus={showMobileSearch} className='pl-2 pr-24 py-1 text-sm text-black bg-white -ml-40
      ...
      <nav className='relative w-full'>
        <div className='block lg:hidden ml-12 -mt-10'>
          <Image src={Logo} alt='Logo' className='w-20 sm:w-36 h-12 mr-4 -mt-6' />
        </div>

        <div className='absolute text-white top-5 right-60  2xl:right-60 whitespace-nowrap  lg:right-4  xl:right-32'>
